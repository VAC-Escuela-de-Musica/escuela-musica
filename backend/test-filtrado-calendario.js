...
async function getAllClasses(token) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/clases/all`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
      credentials: 'include'
    ...
    if (data.data && data.data.length > 0) {
      ...
      console.log('ℹ️  No hay clases en el sistema');
    }
...
async function testFiltradoCalendario() {
  console.log('🚀 Iniciando prueba de filtrado de calendario...\n');
  ...
  if (clasesEstudiante && todasLasClases) {
    ...
    // Verificar que el admin ve todas las clases
    if (todasLasClases.length >= clasesEstudiante.length) {
      console.log('✅ ADMIN FUNCIONANDO: El administrador ve todas las clases');
    } else {
      console.log('⚠️  ADVERTENCIA: El administrador no está viendo todas las clases');
    }
  } else {
...
// Ejecutar la prueba
testFiltradoCalendario().catch(console.error); 