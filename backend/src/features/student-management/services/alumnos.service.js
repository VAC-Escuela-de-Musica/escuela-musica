...
async function createAlumnos(alumno) {
  try {
    ...
    const hashedPassword = await Alumno.encryptPassword(password);
    ...
    console.log(
      "[SERVICE] createAlumnos - Nuevo alumno a guardar:",
      nuevoAlumno,
    );
    ...
    try {
      ...
      console.log(
        "[SERVICE] createAlumnos - Alumno guardado exitosamente:",
        savedAlumno,
      );
      return [savedAlumno, null];
    } catch (saveError) {
      console.error("[SERVICE] createAlumnos - Error al guardar:", saveError);
      console.error(
        "[SERVICE] createAlumnos - Error details:",
        saveError.message,
      );
      if (saveError.errors) {
        console.error(
          "[SERVICE] createAlumnos - Validation errors:",
          saveError.errors,
        );
      }
      return [null, saveError.message || "Error de validación al crear alumno"];
    }
  } catch (error) {
...
async function actualizarEstadoClase(alumnoId, claseId, datosActualizacion) {
  try {
    ...
    const clase = alumno.clases.find((c) => c.clase.toString() === claseId);
