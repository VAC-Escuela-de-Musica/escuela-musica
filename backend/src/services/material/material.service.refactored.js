...
class MaterialService {
  ...
  async getMaterialsWithPagination(params) {
    try {
      ...
      // Agregar filtros de acceso si hay usuario
      if (userId) {
        return await this.repository.findAccessibleMaterials(userId, options);
      }

      return await this.repository.searchMaterials(filters, options);
    } catch (error) {
  ...
  async deleteMaterial(materialId, userId) {
    try {
      ...
      return await this.repository.deleteById(materialId);
    } catch (error) {
   ...
   * @returns {Promise<Result>}
   */
  async toggleFavorite(materialId, userId) {
    try {
      return await this.repository.toggleFavorite(materialId, userId);
    } catch (error) {
      handleError(error, 'MaterialService -> toggleFavorite');
      return Result.error(error.message, 500);
    }
  }
  ...
  async getFavorites(userId, options = {}) {
    try {
      ...
      const mergedOptions = { ...defaultOptions, ...options };
      return await this.repository.getFavorites(userId, mergedOptions);
    } catch (error) {
      handleError(error, 'MaterialService -> getFavorites');
      return Result.error(error.message, 500);
    }
  }
