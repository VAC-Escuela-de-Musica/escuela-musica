...
const WhatsAppConfig = () => {
  ...
  // Cargar estado inicial - solo después de que la autenticación se inicialice
  useEffect(() => {
  ...
  const resetWhatsApp = async () => {
    try {
      setIsLoading(true);
      setAlertInfo({ type: 'info', message: 'Reiniciando WhatsApp Web...' });
      
      const response = await messagingService.resetWhatsAppWeb();
      if (response.success) {
        setAlertInfo({ type: 'success', message: 'WhatsApp Web reiniciado correctamente. Ahora puedes obtener un nuevo código QR.' });
        ...
        setAlertInfo({ type: 'error', message: 'Error reiniciando WhatsApp Web: ' + response.message });
      }
    } catch (error) {
      console.error('Error reiniciando:', error);
      setAlertInfo({ type: 'error', message: 'Error reiniciando WhatsApp Web' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!isInitialized || loading) {
    return (
      <Box sx={{ p: 3, backgroundColor: '#222222', minHeight: '100vh', color: '#ffffff', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Typography variant="h6">Inicializando autenticación...</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ p: 3, backgroundColor: '#222222', minHeight: '100vh', color: '#ffffff' }}>
            ...
            >
              ...
              <AccordionDetails>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  ...
                  >
                    {isLoading ? 'Reiniciando...' : 'Reiniciar Sesión'}
                  </Button>
                </Box>
              </AccordionDetails>
            </Accordion>
            ...
            >
              ...
              <AccordionDetails>
                ...
                <Typography variant="caption" sx={{ color: '#999999', mt: 2, display: 'block' }}>
                  💡 Si aparece "No hay código QR disponible", usa "Reiniciar Sesión" para limpiar la autenticación guardada.
                </Typography>
              </AccordionDetails>
            </Accordion>
      ...
      >
        <DialogTitle sx={{ textAlign: 'center' }}>
          ...
          <Typography variant="subtitle1" component="span">Código QR para Autenticación</Typography>
        </DialogTitle>
